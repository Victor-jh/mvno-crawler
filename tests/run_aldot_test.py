...
logger = logging.getLogger('aldot_test')

async def main():
    loader = PlatformLoader()
    
    # Manually select Aldot
    # Note: Loader uses names from platforms.yaml, which handles the class mapping
    # Ensure config loading logic works in isolation
    
    crawler = loader.get_crawler('aldoot')
    if crawler:
        session_id = datetime.now().strftime('%Y%m%d_%H%M%S')
        print(f"=== Starting AlDot Isolated Test (Session: {session_id}) ===")
        ...
        print("Failed to load AlDot crawler. Check platforms.yaml name.")
