...
logger = logging.getLogger('ktm_test')

async def main():
    loader = PlatformLoader()
    
    # Manually select KTM
    platforms = [('ktmmobile', {'name': 'KT엠모바일', 'base_url': 'https://www.ktmmobile.com', 'enabled': True})]
    
    session_id = datetime.now().strftime('%Y%m%d_%H%M%S')
    print(f"=== Starting KTM Isolated Test (Session: {session_id}) ===")
    
    crawler = loader.get_crawler('ktmmobile')
    if crawler:
        crawler.set_session(session_id)
        print("Crawler loaded. Starting crawl...")
        try:
           # Test with small limit
           await crawler.crawl(headless=True, test_mode=True, limit=2)
        ...
        print("Failed to load KTM crawler.")
