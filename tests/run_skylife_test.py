...
logger = logging.getLogger('skylife_test')

async def main():
    loader = PlatformLoader()
    
    # Manually select SkyLife
    crawler = loader.get_crawler('skylife')
    if crawler:
        session_id = datetime.now().strftime('%Y%m%d_%H%M%S')
        print(f"=== Starting SkyLife Isolated Test (Session: {session_id}) ===")
        ...
        print("Failed to load SkyLife crawler.")
